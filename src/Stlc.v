...
Inductive substi (s : tm) (x : string) : tm -> tm -> Prop :=
  ...
  | s_abs1 : forall (T : ty) (t : tm),
      substi s x <{\x:T, t}> <{\x:T, t}>
  | s_abs2 : forall (y : string) (T : ty) (t1 t2 : tm),
      (x <> y) -> substi s x t1 t2 -> substi s x <{\y:T, t1}> <{\y:T, t2}>
  | s_app : forall (t1 t1' t2 t2': tm),
      substi s x t1 t1' -> substi s x t2 t2' -> substi s x <{t1 t2}> <{t1' t2'}>
  | s_true :
      substi s x <{true}> <{true}>
  | s_false :
      substi s x <{false}> <{false}>
  | s_if : forall (t1 t1' t2 t2' t3 t3': tm),
...
Proof.
  ...
  - intros t'.
    ...
    + intros H.
      rewrite <- H.
      destruct (String.eqb x0 s0) eqn:E1.
      * simpl. rewrite E1. apply eqb_eq in E1. rewrite E1. apply s_var1.
      * simpl. rewrite E1. apply eqb_neq in E1. apply s_var2. apply E1.
    + intros H.
      destruct (String.eqb x0 s0) eqn:E1.
      * simpl. rewrite E1.
        inversion H.
        ** reflexivity.
        ** apply eqb_eq in E1. contradiction.
      * simpl. rewrite E1.
        inversion H.
  ...
  - intros t'.
    ...
    + intros H.
      inversion H.
      * simpl.
        destruct (String.eqb s0 s0) eqn:E1.
        ** reflexivity.
        ** apply eqb_neq in E1. contradiction.
      * simpl.
        destruct (String.eqb x0 s0) eqn:E1.
  ...
  - intros t'. simpl. split.
    + intros H. rewrite <- H. apply s_true.
    + intros H. inversion H. reflexivity.
  - intros t'. simpl. split.
    + intros H. rewrite <- H. apply s_false.
    + intros H. inversion H. reflexivity.
  - intros t'. split.
    + intros H. rewrite <- H. simpl. apply s_if.
      * apply IHt1. reflexivity.
      * apply IHt2. reflexivity.
      * apply IHt3. reflexivity.
    + intros H. inversion H.
      apply IHt1 in H3. rewrite <- H3.
      apply IHt2 in H5. rewrite <- H5.
      apply IHt3 in H6. rewrite <- H6.
      simpl. reflexivity.
Qed.
  (* FILL IN HERE *)
(** [] *)
...
Proof.
  ...
  induction T2.
  - inversion H0.
  - inversion H0. rewrite <- H2 in H1. apply IHT2_1 in H1. apply H1.
Qed.
(** [] *)
