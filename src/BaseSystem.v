...
Inductive reduction : Proc -> Proc -> Prop :=
  | RSend : forall (x y z w: string) (P Q: Proc),
      ~Exist w P -> <{x<y>; Q | x(z); P}> --> <{Q | [z := y][y ::= w]P}>
...
Theorem RRecv : forall (x y z w: string) (P Q: Proc),
  ~Exist w P -> <{x(z); P | x<y>; Q}> --> <{[z := y][y ::= w]P | Q}>.
Proof.
  intros.
  apply RCong with (P' := <{x0<y0>; Q | x0(z0); P}>) (Q' := <{Q | [z0 := y0][y0 ::= w0]P}>).
  - apply multi_step with (y := <{x0<y0>; Q | x0(z0); P}>).
    + apply SComp.
    + apply multi_refl.
  - apply RSend. apply H.
  - apply multi_step with (y := <{[z0 := y0][y0 ::= w0]P | Q}>).
    + apply SComp.
    + apply multi_refl.
Qed.
...
Proof.
  ...
  - apply RNew. apply RComp2. apply RNew.
    apply RRecv with (w := "a").
